...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("RotterdamRP Server Status", "")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ RotterdamRP' : '⚫ RotterdamRP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHoe kan je de server joinen?','Je kan de server joinen doormiddel van **RotterdamRP.nl** in te voeren bij Direct Connect. Onderaan staat de server status om te kijken hoeveel mensen er online zijn en in de wachtrij staan. Dit systeem is gemaakt door RotterdamRP\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('RotterdamRP',{'url':'https://www.twitch.tv/RoqueTV','type':'STREAMING'});
